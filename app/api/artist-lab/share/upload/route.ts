...
export async function POST(request: NextRequest) {
  ...
  for (const file of files) {
    ...
    const cleanName = cleanShareFileName(file.name)
    const key = createShareKey(artistSlug, file.name)
    ...
    if (s3 && bucket) {
      ...
      uploaded.push({ name: file.name, size: file.size, type: file.type || "file", key, url: getSharePublicUrl(key), uploadedAt: new Date().toISOString() })
      continue
    }
