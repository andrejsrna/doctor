...
import { FaMusic, FaPlay, FaPause, FaInfoCircle, FaArrowLeft, FaArrowRight, FaSearch, FaVirus, FaBox } from 'react-icons/fa'
...
export default function MusicPacksPage() {
  ...
  const [musicPacksCategoryId, setMusicPacksCategoryId] = useState<string>('')
  const postsPerPage = 12
  const sectionRef = useRef<HTMLElement>(null)

  // Use SWR hooks with caching
  const { data: postsData, isLoading: postsLoading } = useLatestPosts(postsPerPage, currentPage, musicPacksCategoryId, searchQuery)
  ...
  // Find Music Packs category ID
  useEffect(() => {
    const musicPacksCategory = categories.find((cat: Category) => 
      cat.name.toLowerCase().includes('music pack') || 
      cat.slug.toLowerCase().includes('music-pack')
    )
    if (musicPacksCategory) {
      setMusicPacksCategoryId(musicPacksCategory.id.toString())
    }
  }, [categories])
  ...
  useEffect(() => {
    setCurrentPage(1)
  }, [musicPacksCategoryId, searchQuery])

  useEffect(() => {
    if (sectionRef.current) {
      sectionRef.current.scrollIntoView({ behavior: 'smooth' })
    }
  }, [currentPage, musicPacksCategoryId, searchQuery])
    ...
    <>
      ...
      <div className="relative h-[70vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <Image
            src="/music-bg.jpeg"
            alt="DnB Doctor Music Packs Background"
            fill
